...
For every generated JAM truth member (original ``Q``) and every test mode
...
consistently whether the truth was JAM read at ``mc``, ``1``, ``2``, ``3``,
``4``, or ``5 GeV``.

Run standalone::

    python -m closure_JAM_truth.run_closure                 # every generated Q, all modes
    python -m closure_JAM_truth.run_closure --Q 2            # one member
    python -m closure_JAM_truth.run_closure --modes both     # one mode
"""
...
def plot_reproduction(q_key, mode, marginal, truth, path):
    """3x3 grid: injected JAM truth vs recovered posterior as ``x * PDF``."""
    _prepare_matplotlib()
    import matplotlib
    matplotlib.use("Agg")
    ...
    for ax, name in zip(axes.ravel(), cfg.ALL_FIELDS):
        ...
        y_truth = tcurve
        # True error band: the JAM replica-ensemble spread of the truth field.
        ...
        ax.fill_between(xn, y_truth - t_std, y_truth + t_std,
                        ...
                        label="x * JAM truth $\\pm$ replica error")
...
def run_one(q_key, mode, *, make_plots=True, write_outputs=True, run_cache=None):
    ...
    if write_outputs:
        ...
        for layout in moments.q_layout:
            name = str(layout["name"])
            field_nodes = np.asarray(field_by_name[name].nodes, dtype=float)
            node_parts.append(field_nodes)
            curve = (truth.get("curves") or {}).get(name)
            truth_parts.append(
                np.full(field_nodes.shape, np.nan)
                if curve is None
                else np.interp(field_nodes, truth_nodes, np.asarray(curve, dtype=float))
            )
        save_posterior_moments(
            out / f"posterior_moments_{mode}.npz",
            moments,
            ...
            metadata={
                ...
                "q_key": q_key,
                "mode": mode,
                "n_physical_data": summary["n_physical_data"],
                "n_constraint_data": summary["n_constraint_data"],
                "chi2_fit": summary["chi2"],
            },
        )
    if run_cache is not None:
        run_cache.setdefault(mode, []).append((q_key, result["marginal"], truth))
    t_pdf_plot = t_data_plot = t_extent_plot = 0.0
    if make_plots:
        try:
        ...
        try:
            result["analysis"].plot_kinematic_coverage(
                path=out / f"kinematic_coverage_{mode}",
                close=True,
            )
        except Exception as exc:  # plotting is optional
            print(f"   [kinematic-coverage plot skipped: {exc}]")
        t_extent_plot = watch.lap()
    ...
    if make_plots:
        ...
        parts.append(f"extent-plot {format_duration(t_extent_plot)}")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run JAM closure fits.")
    ...
    if not qs:
        raise SystemExit("no generated truth members; run closure_JAM_truth.generate first")
    ...
    for q_key in qs:
        for mode in args.modes:
            ...
            print(f"   chi2/n={s['chi2_per_data']:.3g}  n={s['n_data']}  "
                  ...
                  f"|pull|<=2={n_le_2}")
            nuis = s.get("nuisance_coverage") or {}
            if nuis:
                nfit = sum(1 for v in nuis.values()
                           if (v.get("frac_pull_le_2") or 0.0) >= 0.9)
                print(f"   lattice systematics: {len(nuis)} nuisance fields, "
                      f"{nfit} recovered (>=90% within 2sigma)")
