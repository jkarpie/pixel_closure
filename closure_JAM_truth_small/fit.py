...
the posterior band covers the injected JAM truth.
"""
...
from pixel.infer import PosteriorResult, NestedVegasSamples, VegasImportanceSamples
from pixel.infer import AutocorrelationWarning, autocorrelation_summary
from pixel.infer import run_mcmc as infer_mcmc
from pixel.infer import bilinear_posterior_moments
...
def build_analysis(q_key: str, mode: str, *, use_kernel_cache: bool = True, t0=None):
    """Construct the Analysis for ``(q_key, mode)`` and freeze all GP cov params.

    Returns ``(analysis, fields)``.  After freezing, the only free parameters
    are any non-frozen model parameters; for the closure prior this is zero.
    """
    ...
    for ds in dsets.build_datasets(
        ...
        include_dy=mode in ("dy", "exp", "both"),
        include_synthetic_z=mode == cfg.TEST_MODE_SYNTHETIC_Z,
    ...
    analysis.compile(rcond=cfg.RCOND)
    # Freeze the constant prior mean N plus sigma, length and the x_reg
    # regularizer, so the prior is fully specified and there are no free params.
    frozen = []
    for name in cfg.ALL_FIELDS:
...
# -- run: VEGAS over fitted normalizations -----------------------------------


def _closure_sampler(model, mode: str | None = None) -> str:
    """Use VEGAS for every sampled small-closure parameter block.

    The public dispatcher resolves linear models to ordinary VEGAS and
    DY-containing models to nested VEGAS.  A lattice-only model has no free
    coordinates and therefore resolves to the analytic fixed posterior without
    drawing artificial samples.

    ``synthetic_z_plumbing`` is the exception.  It is a single unevolved
    gamma/Z row carrying no ordinary theta and no fitted normalization, so once
    the field is marginalized the only coordinate left is the
    Hubbard-Stratonovich auxiliary -- which VEGAS refuses outright ("needs at
    least one outer theta or normalization coordinate when f is marginalized"),
    failing every member of the mode.  Its own message names the alternatives;
    ``gaussian`` is the cheapest, and since this mode is plumbing rather than
    physics validation the sampler choice carries no physics consequence.
    """
    if mode == cfg.TEST_MODE_SYNTHETIC_Z:
        return "gaussian"
    return "vegas"
...
def run_fit(q_key: str, mode: str, *, use_kernel_cache: bool = True,
    ...
    """
    watch = Stopwatch()
    # ``synthetic_z_plumbing`` cannot run at the configured MCMC_SAMPLES: its
    # sampler is quadratic in both time and memory and the 1000-sample fit is
    # SIGKILLed at ~23 GB.  See cfg.SYNTHETIC_Z_MCMC_SAMPLES for the measurements.
    if mode == cfg.TEST_MODE_SYNTHETIC_Z:
        n_samples = min(int(n_samples), int(cfg.SYNTHETIC_Z_MCMC_SAMPLES))
    ...
    t_build = watch.lap()
    sampler = _closure_sampler(model, mode)
    if mode == cfg.TEST_MODE_SYNTHETIC_Z and model.has_bilinear:
        # ``synthetic_z_plumbing`` carries a single bilinear row with no ordinary
        # theta and no fitted normalization, so marginalizing f leaves the
        # sampler no outer coordinate at all and it refuses outright.  Sample the
        # one Hubbard-Stratonovich auxiliary instead; a single auxiliary
        # dimension is the regime that works today.  That routes to
        # ``saddle_contour_importance_sample``, whose signature shares only
        # ``saddle_tol`` with the VEGAS options below -- passing the rest is a
        # TypeError, so this branch builds its own minimal set.  Cost grows
        # quadratically in the sample count here: measured 7.1s at 128, 18.6s at
        # 256, 32.5s at 384, so the configured 1000 is a handful of minutes.
        auxiliary = "sample"
        sampler_options = {"saddle_tol": cfg.CONTOUR_SADDLE_TOL}
    else:
        sampler_options = {
            "n_bins": cfg.VEGAS_N_BINS,
            "n_adapt_iterations": cfg.VEGAS_N_ADAPT_ITERATIONS,
            "n_eval_per_iteration": cfg.VEGAS_N_EVAL_PER_ITERATION,
            "alpha": cfg.VEGAS_ALPHA,
            "covariance_inflation": cfg.VEGAS_COVARIANCE_INFLATION,
        }
        auxiliary = "auto"
        if model.has_bilinear:
            auxiliary = "marginalize"
            sampler_options["covariance_inflation"] = (
                cfg.VEGAS_JOINT_COVARIANCE_INFLATION
            )
            sampler_options["n_inner_samples"] = cfg.VEGAS_N_INNER_SAMPLES
            sampler_options["n_adapt_inner_samples"] = (
                cfg.VEGAS_N_ADAPT_INNER_SAMPLES
            )
            sampler_options["joint_grid"] = cfg.VEGAS_JOINT_GRID
            sampler_options["saddle_tol"] = cfg.CONTOUR_SADDLE_TOL
            sampler_options["min_adapt_phase_ess_frac"] = (
                cfg.VEGAS_MIN_ADAPT_PHASE_ESS_FRAC
            )
            sampler_options["max_extra_adapt_iterations"] = (
                cfg.VEGAS_MAX_EXTRA_ADAPT_ITERATIONS
            )
    reporter.emit(
        f"inference start sampler={sampler} auxiliary={auxiliary} "
        f"n_samples={int(n_samples)}"
    )
    samples = infer_mcmc(
        model, sampler=sampler, auxiliary=auxiliary,
        n_samples=n_samples, run_map=False, seed=seed,
        **sampler_options,
    )
    if isinstance(samples, VegasImportanceSamples):
        ess_frac = samples.effective_sample_size / samples.n_samples
        if ess_frac < cfg.VEGAS_MIN_OUTER_ESS_FRAC:
            raise RuntimeError(
                "small-closure VEGAS ESS fraction "
                f"{ess_frac:.3g} is below {cfg.VEGAS_MIN_OUTER_ESS_FRAC}; "
                "refusing an unreliable fit"
            )
    elif isinstance(samples, NestedVegasSamples):
        absolute_frac = samples.absolute_ess / samples.n_samples
        signed_frac = samples.signed_ess / samples.n_samples
        production_median_inner = float(np.median(samples.inner_ess_frac))
        if samples.metadata.get("contour_strategy") == "joint_affine_grid":
            # Early pilot iterations intentionally start from an unadapted
            # proposal.  Their ESS determines later grids but is not a quality
            # measure for the independent production sample.
            median_inner = production_median_inner
        else:
            adaptation_median_inner = min(
                (float(row["median_inner_ess_frac"])
                 for row in samples.adaptation_history),
                default=production_median_inner,
            )
            median_inner = min(
                production_median_inner, adaptation_median_inner
            )
        failures = []
        if absolute_frac < cfg.VEGAS_MIN_OUTER_ESS_FRAC:
            failures.append(
                f"absolute outer ESS fraction {absolute_frac:.3g} < "
                f"{cfg.VEGAS_MIN_OUTER_ESS_FRAC}"
            )
        if signed_frac < cfg.VEGAS_MIN_SIGNED_ESS_FRAC:
            failures.append(
                f"signed outer ESS fraction {signed_frac:.3g} < "
                f"{cfg.VEGAS_MIN_SIGNED_ESS_FRAC}"
            )
        if median_inner < cfg.VEGAS_MIN_MEDIAN_INNER_ESS_FRAC:
            failures.append(
                f"median inner ESS fraction {median_inner:.3g} < "
                f"{cfg.VEGAS_MIN_MEDIAN_INNER_ESS_FRAC}"
            )
        if failures:
            raise RuntimeError(
                "small-closure nested VEGAS diagnostics failed: "
                + "; ".join(failures)
            )
    reporter.emit(f"inference done sampler={sampler}", lap=True)
    t_mcmc = watch.lap()
    reporter.emit("posterior marginalization start")
    marginal = marginal_posterior(model, samples)
    reporter.emit("posterior marginalization done", lap=True)
    t_marginal = watch.lap()
    reporter.emit("posterior predictive chi2 start")
    vec_bar = _weighted_mean_vec(samples, model=model)
    chi2_components = {"linear": None, "bilinear": None}
    try:
        if model.has_bilinear and hasattr(samples, "mean"):
    ...
    return {
        ...
        "samples": samples, "marginal": marginal, "vec_bar": vec_bar,
        ...
        "sampler": sampler,
        "sampler_diagnostics": (
            samples.summary() if hasattr(samples, "summary") else {}
        ),
...
def _weights(samples) -> np.ndarray:
    ...
    for name in (
        "normalized_signed_weights", "residual_weights", "phase_weights", "weights"
    ):
...
def _weighted_mean_vec(samples, model=None) -> np.ndarray:
    if isinstance(samples, PosteriorResult):
        return np.asarray(samples.x, dtype=float)
    if isinstance(samples, NestedVegasSamples):
        theta_mean = np.asarray(samples.theta_mean, dtype=float)
        if model is None:
            return theta_mean
        partition = model.free_partition()
        return np.asarray(
            model.merge_free_vector(
                theta_mean, np.zeros(partition.n_hubbard, dtype=float)
            ),
            dtype=float,
        )
...
def _effective_sample_size(samples) -> float:
    ...
    if isinstance(samples, PosteriorResult):
        return 1.0
    if isinstance(samples, NestedVegasSamples):
        return float(samples.signed_ess)
...
def _markov_chain(samples):
    ...
    * A fixed-parameter ``PosteriorResult`` has no chain at all.
    ...
    if isinstance(samples, PosteriorResult):
        return None
    if isinstance(samples, (VegasImportanceSamples, NestedVegasSamples)):
        return None
...
def bilinear_marginal_posterior(model, samples) -> dict:
    ...
    integral over the auxiliary ``f``; a plain weighted average of the fixed-``f``
    linear posteriors is wrong.  Use the canonical phase-aware accumulators over
    the sampler output, then split the concatenated GP-grid result per field.
    """
    if isinstance(samples, NestedVegasSamples) or hasattr(samples, "ess_frac"):
        # Contour-HMC artifacts already contain the correctly residual-weighted
        # physical moments, including each flow Jacobian.  Joint/nested VEGAS
        # likewise stores its already-integrated physical moments directly.
...
def marginal_posterior(model, samples) -> dict:
    ...
    is the total ``E[var_s] + Var[mu_s]`` (aleatoric + epistemic).  When the
    model carries bilinear (Drell-Yan / H-S) data the field posterior is the
    complex-phase ``f``-integral, so dispatch accordingly.
    """
...
def fitted_normalization_posterior(model, samples) -> dict:
    ...
    if isinstance(samples, NestedVegasSamples):
        label_to_index = {label: i for i, label in enumerate(samples.theta_labels)}
        out = {}
        for owner, full_index in pairs:
            label = model.free_labels()[full_index]
            if label not in label_to_index:
                continue
            i = label_to_index[label]
            out[owner] = {
                "mean": float(samples.theta_mean[i]),
                "std": float(np.sqrt(max(samples.theta_covariance[i, i], 0.0))),
            }
        return out
...
def summarize(q_key, mode, result, truth) -> dict:
    ...
    return {
        ...
        "sampler_diagnostics": result.get("sampler_diagnostics", {}),
