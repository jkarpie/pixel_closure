"""Fold the NNPDF replica ensemble through the PIXEL forward operators and save data.

For one original scale ``Q`` (``q_key``):

1. evaluate every NNPDF replica (members ``1..1000``) on the shared field nodes
...
3. build the PIXEL datasets with :mod:`closure_NNPDF_truth_small.datasets` -- the
   *same* builders the fit uses -- and fold **every replica** through them
...
Run standalone::
    ...
    python -m closure_NNPDF_truth_small.generate --Q 2          # one member
    python -m closure_NNPDF_truth_small.generate --all          # every Q in config
    python -m closure_NNPDF_truth_small.generate --random        # a random Q, seeded
"""
...
# -- NNPDF replica ensemble ----------------------------------------------------


def _ensemble_cache_path(q_key: str) -> Path:
    return cfg.REFERENCE_DIR / f"nnpdf_ensemble_{cfg.truth_label(q_key)}.npz"


def ensemble_curves(q_key: str, nodes, *, members=cfg.NNPDF_REPLICA_MEMBERS,
                    use_cache: bool = True):
    """Return ``(curves, meta)`` for the NNPDF replica ensemble at scale ``q_key``.

    ``curves`` maps each field to a ``(n_replicas, n_nodes)`` array of the
    projected truth values (one row per replica).  Cached to
    ``reference_pdfs/nnpdf_ensemble_<label>.npz`` so re-generation does not re-run
    ...
    meta = None
    for m in members:
        curves_m, meta = pdf_guidance.nnpdf_truth_curves(nodes, q_key=q_key, member=m)
...
def generate_member(q_key: str, *, noise: bool = True, seed: int = 20260710,
    ...
    The truth field is the NNPDF replica-ensemble **mean**; the fake-data
    ...
    # (1) NNPDF replica ensemble -> mean truth curves + per-node spread.
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Generate NNPDF closure data.")
    ...
    for q_key in keys:
        print(f"== generating truth member Q={cfg.TRUTH_Q_CHOICES[q_key]} "
              ...
              f"{len(cfg.NNPDF_REPLICA_MEMBERS)} NNPDF replicas ==")
        watch = Stopwatch()
