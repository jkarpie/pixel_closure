"""Evaluate the NNPDF truth and project it into the nine closure basis fields.

The truth is the real ``NNPDF40_nnlo_as_01180_1000`` LHAPDF set.  There is no
python-LHAPDF binding here, so NNPDF is evaluated with the small C++ helper
``closure_NNPDF_truth_small/lhapdf_dump.cc`` (compiled once against the local LHAPDF
install).

For each *original* scale ``Q`` we read NNPDF at ``Q`` on the closure x grid,
...
Run standalone to cache every truth member and report fit quality::
    ...
    python -m closure_NNPDF_truth_small.pdf_guidance
"""
...
def ensure_lhapdf_data() -> Path:
    """Return an LHAPDF data root containing the NNPDF grid set.

    Prefers a set already present in the LHAPDF install; otherwise extracts the
    repo-vendored tarball into :data:`config.NNPDF_LHAPDF_DATA_ROOT` once.
    ...
    """
    installed = cfg.LHAPDF_DATA_DIR / cfg.NNPDF_SET / f"{cfg.NNPDF_SET}.info"
    if installed.exists():
        return cfg.LHAPDF_DATA_DIR

    set_dir = cfg.NNPDF_LHAPDF_DATA_ROOT / cfg.NNPDF_SET
    info_path = set_dir / f"{cfg.NNPDF_SET}.info"
    if info_path.exists():
        return cfg.NNPDF_LHAPDF_DATA_ROOT

    if not cfg.NNPDF_TARBALL.exists():
        raise FileNotFoundError(f"NNPDF LHAPDF tarball not found: {cfg.NNPDF_TARBALL}")

    cfg.NNPDF_LHAPDF_DATA_ROOT.mkdir(parents=True, exist_ok=True)
    root = cfg.NNPDF_LHAPDF_DATA_ROOT.resolve()
    with tarfile.open(cfg.NNPDF_TARBALL, "r:gz") as tar:
        for member in tar.getmembers():
            dest = (cfg.NNPDF_LHAPDF_DATA_ROOT / member.name).resolve()
            if dest != root and root not in dest.parents:
                raise ValueError(f"unsafe LHAPDF archive member: {member.name}")
        tar.extractall(cfg.NNPDF_LHAPDF_DATA_ROOT)

    if not info_path.exists():
        raise FileNotFoundError(
            f"NNPDF LHAPDF archive did not create expected info file: {info_path}"
        )
    return cfg.NNPDF_LHAPDF_DATA_ROOT


def dump_nnpdf(nodes: np.ndarray, *, q: float, member: int = cfg.NNPDF_MEMBER) -> np.ndarray:
    ...
    proc = subprocess.run(
        [str(exe), cfg.NNPDF_SET, str(int(member)), f"{float(q):.17g}"],
...
def nnpdf_truth_curves(nodes, *, q_key: str, member: int = cfg.NNPDF_MEMBER):
    """Return ``(curves, meta)`` for NNPDF at original scale ``q_key``.
    ...
    raw = dump_nnpdf(np.asarray(nodes, dtype=float), q=q, member=member)
    curves = project_basis(raw)
    meta = {
        "set": cfg.NNPDF_SET, "member": int(member), "q_key": q_key,
        "q_original": q, "q_effective": q,
        "below_set_qmin": bool(
            cfg.NNPDF_QMIN is not None and q < cfg.NNPDF_QMIN
        ),
...
def main() -> None:
    ...
    compile_lhapdf_dump()
    print(f"NNPDF set: {cfg.NNPDF_SET}  (member {cfg.NNPDF_MEMBER})  input Q0=mc={cfg.Q0}")
    for q_key in cfg.TRUTH_Q_CHOICES:
        curves, meta = nnpdf_truth_curves(nodes, q_key=q_key)
        ...
        path = cfg.REFERENCE_DIR / f"nnpdf_{cfg.truth_label(q_key)}.json"
