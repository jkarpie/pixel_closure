# Full-flavor closure test: lattice + DIS + Drell-Yan (NNPDF truth)
...
[`closure_NNPDF_truth/`](../closure_NNPDF_truth) — 13 real lattice ensembles, full DIS
...
The separate ``nnlo_dis_smoke.py`` route is the deliberately small exception:
it folds the vendored NNPDF4.0 replica ensemble through two F2, two HERA NC
``sigma_r``, and two HERA CC ``sigma_r`` rows using massless MSbar NNLO
F2/FL/F3 coefficients and a declared ``fixed_nf=4`` evolution baseline. It
exercises both plus/minus charged-current channels. Generalized-NC rows remain
excluded until their three-loop FL ``fl11`` vector/axial charge factor is
implemented. Run it explicitly; it is not part of the routine or large closure
campaign.

Likewise, ``nnlo_dy_smoke.py`` keeps the ordinary DY campaign at LO but adds an
explicit one-row correctness closure for the complete pp virtual-photon NNLO
rapidity block. It uses real E866 kinematics and the cached NNPDF4.0 NNLO
replica ensemble, requires a user-supplied ReDY v0.3 archive (or
``PIXEL_REDY_SOURCE``), closes the public builder at a cheap smoke quadrature,
and reports a second refinement separately. Run it explicitly; it does not run
the broader closure studies and its smoke prediction is not a production
benchmark.

``nnlo_heavy_smoke.py`` is the corresponding focused massive-DIS check. It
uses two real HERA combined charm reduced-cross-section kinematic rows from the
NNPDF commondata checkout, folds the NNPDF4.0 replica truth through the exact
fixed-``nf=3`` FFNS ``F2`` and ``FL`` coefficients through ``alpha_s^2``, and
rebuilds the public ``HeavyNCSigmaRFFNS`` dataset. The same two kinematic rows
also close the S-ACOT-chi and FONLL-A heavy reduced cross sections on the NNPDF
`nf=4` active-charm basis, exercising independent F2/FL kernels and the physical
row-wise inelasticity factor. The selected Q2=12 and 200 GeV2 rows lie on
opposite sides of the bottom boundary, so the GM-VFNS routes also exercise the
matched Nf=4-to-Nf=5 T24 reconstruction.
Run it explicitly with
``python -m closure_NNPDF_truth_small.nnlo_heavy_smoke``. This validates the
Pixel operator and reduced-cross-section assembly; it is deliberately not
labelled NNPDF theory-200 FONLL-C/TMC parity.

See the repo-root `closure_plan.md` for the full design narrative.

## Truth = real NNPDF, read across scales

The truth is the real `NNPDF40_nnlo_as_01180_1000` global-analysis result
(NNLO LHAPDF grid, used with PIXEL's NLO closure kernels), used **directly**:
every replica (members `1..1000`) is projected into the nine C-even/C-odd basis
fields and folded through the operators as-is.  The **truth field is the ensemble mean** over replicas and the
**fake-data covariance is the replica covariance** of the folded output — the
real NNPDF uncertainty propagated through the forward model, not an invented error
budget.
...
We build several truth members by reading NNPDF at different **original scales**
...
fake data are generated).  Reading NNPDF at different `Q` yields genuinely
...
`Q = 1 GeV` and `Q = mc` are evaluated directly through LHAPDF; the truth record
notes that these are extrapolations below NNPDF's advertised `QMin = 1.65`.

The NNPDF LHAPDF grids are expected at
`~/Downloads/NNPDF40_nnlo_as_01180_1000.tar.gz`.  The first NNPDF evaluation
extracts that archive into `closure_NNPDF_truth_small/data/lhapdf/` and sets
`LHAPDF_DATA_PATH` to that extracted data root for the C++ dumper.
...
All fake-data errors come from the **NNPDF replica spread of the folded observable**
...
- **Physics constraints** (near-hard pseudo-data, every mode):
  ...
  - valence quark-counting normalizations from `∫(u−ū)=2, ∫(d−d̄)=1,
    ...
    moment).  The NNPDF truth already satisfies these, so they are
    closure-consistent.
...
| `pdf_guidance.py` | evaluate NNPDF (via `lhapdf_dump.cc`), project to 9 fields, reference fit |
| `truth.py` | assemble a truth member (NNPDF curves + reference fit) |
| `datasets.py` | build PIXEL datasets from a member's manifest (shared by gen + fit) |
| `generate.py` | fold every NNPDF replica through the operators, take ensemble mean/covariance, write data; DY folds the single ensemble-mean truth curve, not a replica statistic |
...
Use the repo-level wrapper with `--truth NNPDF` for normal runs:

```bash
./run_closure.sh --truth NNPDF --Q 2 --modes both
./run_closure.sh --truth NNPDF --Q 2 --remake-data --remake-kernels
./run_closure.sh --truth NNPDF --all --modes lattice,dis,dy,exp,both
```
...
./run_closure.sh --truth NNPDF --all --data-only --remake-data
```
...
```bash
./run_closure.sh --truth NNPDF --Q 2 --modes both --kernels-only
./run_closure.sh --truth NNPDF --all --kernels-only --remake-kernels
```
...
selected `results/` outputs first unless `--keep-results` is supplied.  On the
first data-generation run, the NNPDF LHAPDF tarball is extracted from
`~/Downloads/NNPDF40_nnlo_as_01180_1000.tar.gz` into
`closure_NNPDF_truth_small/data/lhapdf/`.

The underlying modules are still useful when debugging a specific stage:

```bash
venv/bin/python -m closure_NNPDF_truth_small.dis_audit                 # 1. audit real DIS tables
venv/bin/python -m closure_NNPDF_truth_small.pdf_guidance              # 2. cache NNPDF truth per Q
venv/bin/python -m closure_NNPDF_truth_small.generate --all            # 3. write data for every Q
#   or a single member:  venv/bin/python -m closure_NNPDF_truth_small.generate --Q 2
venv/bin/python -m closure_NNPDF_truth_small.run_closure               # 4. fit all modes + compare
#   or one:              venv/bin/python -m closure_NNPDF_truth_small.run_closure --Q 2 --modes both
```

Outputs land under `closure_NNPDF_truth_small/data/truthQ_<label>/` (truth + fake data) and
`closure_NNPDF_truth_small/results/{truthQ_<label>,comparison}/` (summaries + PNG/PDF figures).
...
in the shared `closure_NNPDF_truth_small/data/_kernel_cache/` for reuse across processes (e.g. the
fits).
...
`closure_NNPDF_truth_small/data/_kernel_cache/` wipe: the next fit re-derives sampled rows, and
...
```bash
./run_closure.sh --truth NNPDF --Q 2 --trust-kernel-cache   # accept the fingerprint with no content check at all
./run_closure.sh --truth NNPDF --Q 2 --verify-kernel-cache  # always re-derive sampled rows; the strongest and by far the slowest check
```
...
- The `t15/v15` "charm" and `v8` "strange-valence" NNPDF inputs are small/near-zero
...
`closure_NNPDF_truth_small/data/standard/` plus `standard.py` / `generate_standard.py` are an
