...
(:data:`closure_NNPDF_truth.config.ANALYSIS_IDIS_IDX`), with **all** retained rows
...
:data:`closure_NNPDF_truth.config.FITPACK_IDIS` and records its status, so the
...
:mod:`closure_NNPDF_truth.generate` reads; ``manifest["audit"]`` holds the full
scan of every workbook.

Run standalone::

    python -m closure_NNPDF_truth.dis_audit
"""
