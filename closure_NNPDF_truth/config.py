...
the full-scale design, and ``closure_NNPDF_truth/README.md`` for usage.
...
# rather than silently entering with a wrong inclusive proxy.  The returned
# records carry the native matching route and implementation gaps as well as
# the NNPDF reference identity.
...
_CONTRACT_SELECTION = _full_closure_selection("nnpdf_full")
...
#: Original NNPDF scales used to build distinct truth members.  Each is read from
#: NNPDF at this Q, then assigned as the truth at ``Q0 = mc``.  ``Q = 1`` and
#: ``Q = mc`` are evaluated directly through LHAPDF even though they are below
#: ``NNPDF_QMIN``.
TRUTH_Q_CHOICES = {
    "mc": MC,
    "1": 1.0,
...
# Complete the unresolved interval 0 <= x < X_MIN for every x*f field.  The
# singlet/gluon momentum densities may approach a constant; nonsinglet and
# valence combinations must vanish for their zeroth moments to converge.
...
#: The lattice route is the reduced pseudo-ITD ratio, matched in MSbar.  These
#: are explicit closure inputs rather than undocumented defaults so a future
#: RI/MOM or hybrid closure cannot reuse this NLO coefficient by accident.
...
#: observable ("integrate each true sample"): fold every NNPDF replica, take the
...
CONSTRAINT_NORM_SIGMA = 1.0e-4                    # ~hard: valence integrals
