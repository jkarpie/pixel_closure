"""Fold one fixed NNPDF truth through the PIXEL forward operators and save data.

For one original scale ``Q`` (``q_key``):

1. evaluate one frozen NNPDF member on the shared field nodes (assigned at
...
3. build the PIXEL datasets with :mod:`closure_NNPDF_truth.datasets` (the *same*
...
Run standalone::
    ...
    python -m closure_NNPDF_truth.generate --Q 2          # one member
    python -m closure_NNPDF_truth.generate --all          # every Q in config
    python -m closure_NNPDF_truth.generate --random        # a random Q, seeded
"""
...
def ensemble_curves(q_key: str, nodes, *, members=cfg.NNPDF_REPLICA_MEMBERS,
    ...
    replica).  Cached to ``reference_pdfs/nnpdf_ensemble_<label>.npz``.
    """
...
def generate_member(q_key: str, *, noise: bool = True, seed: int = 20260710,
    ...
    # (1) One representable NNPDF truth.  The replica ensemble is retained only
    # as an external uncertainty source for the fixed lattice covariance.
    ens, covariance_meta = ensemble_curves(q_key, nodes)
    mean_curves, meta = pdf_guidance.nnpdf_truth_curves(
    ...
    manifest = {
        ...
        "closure_extension": closure_extension_roster("nnpdf_full"),
